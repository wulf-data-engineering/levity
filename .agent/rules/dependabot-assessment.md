...
description: Instructions and evaluation criteria for assessing Dependabot PR safety and auto-merge eligibility in CI for the template repository
---

# Dependabot Safety Assessment Context

You are running headless in an automated GitHub Actions CI workflow to evaluate whether a passing Dependabot pull request on the Levity template repository is safe to be auto-merged into the repository, or whether it introduces risks that require human review.
...
- All automated verification checks — including generating a fresh template instance, unit tests, end-to-end tests, linting, and protocol checks — have already executed and passed with 100% success in CI before this assessment was triggered.
...
- You **CAN** and **SHOULD** perform targeted searches (`grep_search` or `find_by_name`) inside the template codebase (`template/@@cookiecutter.project_slug@@/frontend/`, `template/@@cookiecutter.project_slug@@/backend/`, `template/@@cookiecutter.project_slug@@/infrastructure/`, `tools/`) to verify whether affected symbols, types, or newly introduced patterns are actually used.
...
1. **Targeted Codebase Search**:
   Quickly check if the affected types, functions, or patterns are actually referenced or used in the template code (`template/@@cookiecutter.project_slug@@/` or `tools/`).
2. **If NOT used in our codebase**:
   If our template code does not reference the affected APIs/types/variants at all, the change has no impact on applications generated from this template. Since CI and tests pass, mark as **`safe: true`**.
3. **If USED in our codebase**:
   Since CI, tests, and compiler/linters (e.g., Rust `cargo check` / TypeScript `tsc`) passed without error on the generated template instance, the code compiles cleanly. In Rust, this means any match statements covering the enum/type include a catch-all pattern (`_ => ...`).
...
1. **Unresolved Breaking Changes in Used Code**:
   - Breaking API changes or removed functions directly affecting code in the template or tools.
...
3. **Toolchain & Compiler Updates**:
   - Rust toolchain upgrades (`rust-toolchain` in `template/@@cookiecutter.project_slug@@/backend/` or `tools/`).
