...
#### Protobuf Serialization & Deserialization (Serde gotchas with flatten)

When Protocol Buffer structs (generated by `prost-build` and annotated with `serde` attributes in `backend/build.rs`) are stored in DynamoDB, they are often wrapped in a wrapper struct (e.g., `Versioned<T>`) and deserialized using `#[serde(flatten)]` on the inner data.

**CRITICAL GOTCHA**: Due to a known Serde limitation (Serde issue #1618), container-level attributes like `#[serde(default)]` aare ignored on flattened structs. This means if new fields are added to a `.proto` file, deserialization of older DynamoDB entries lacking these keys will fail with a `missing field` error.

To prevent this, you **MUST** add explicit field-level default attributes in `backend/build.rs` for any fields that might be missing from older database records:
```rust
config.field_attribute("package_name.MessageName.new_field", "#[serde(default)]");
```
...
When you create a new backend lambda, make sure it has the correct IAM permissions:
...
For lambdas that handle queries or commands for authorized users, set the `authorizer`:
...
**Important:** Be defensive. Set the authorizer if not specified otherwise. Ask the developer if the lambda is not intended for authorized users than skip the authorizer on your own.
