...
**CRITICAL GOTCHA**: Due to a known Serde limitation (Serde issue #1618), container-level attributes like `#[serde(default)]` are ignored on flattened structs. This means if new fields are added to a `.proto` file, deserialization of older DynamoDB entries lacking these keys will fail with a `missing field` error.
