...
**Important:** Applying this skill has some implications on the architecture. Discover the project and its architecture before applying it. 
...
```yaml
  websocket-mock:
    ...
    environment:
      - CARGO_LAMBDA_WATCH_TARGET=host.docker.internal:${BACKEND_PORT:-9000}
    extra_hosts:
...
**CRITICAL WARNING:** 
Do **NOT** define background workers (e.g. SQS consumers, EventBridge targets, chron jobs, or the `processorFunction` below) inside `infrastructure/lib/constructs/backend/api.ts`! 
The `Api` construct is entirely skipped in local development (`aws = false`), meaning background workers defined there will never be deployed or mapped to their event sources locally. 
You MUST define background workers in `infrastructure/lib/constructs/backend.ts` instead. Only Lambdas directly triggered by API Gateway should reside in `api.ts`.

**Always verify your implementations against this skill's exact examples.** Do not pass a feature based on this skill over to the user without making sure it works locally.

### 1. CDK Construct: Backend
In `infrastructure/lib/constructs/backend.ts`:

Add the `VersionedTable` import and DynamoDB imports:
```typescript
...
```typescript
    workerLambda.addEnvironment('WEBSOCKET_API_URL', this.webSocketUrl || `http://localhost:${process.env.WEBSOCKET_MOCK_PORT || '3001'}`);
```

**CRITICAL:** Ensure you place any background worker definitions in `backend.ts` **OUTSIDE** of the `if (deploymentConfig.aws)` conditional block. If placed inside, they will not be deployed or triggered locally via LocalStack.
...
```typescript
        if (dev) {
            cachedConfig = {
                ...
                webSocketUrl: import.meta.env.VITE_WEBSOCKET_URL || `ws://localhost:${import.meta.env.VITE_WEBSOCKET_MOCK_PORT || '3001'}/websocket` /* NEW */
            };
        }
```
...
**CRITICAL** To complete the setup, document the new port in your project root `.env.example`:

```env
WEBSOCKET_MOCK_PORT=3001
```

If there is a `.env` file, ask the user which port to use for `WEBSOCKET_MOCK_PORT` and add it to the `.env` file.
...
const webSocketUrl = dev ? this.webSocketUrl || `http://localhost:${process.env.WEBSOCKET_MOCK_PORT || '3001'}` : this.webSocketUrl!; // API is not available locally
processorFunction.addEnvironment('WEBSOCKET_API_URL', webSocketUrl); 
...
**Always verify your implementations against this skill.** 
Check your implementation step-by-step against the instructions provided here.
Take a look at all instructions in this skill and verify you followed all of them.

**Verify that your implementation is running.** 
Do not pass a new feature based on this skill over to the user without making sure it works locally. Test the integration end-to-end to ensure the background processing and WebSocket responses flow correctly before completing your task.
Use @../../workflows/run-locally.md to start the environment