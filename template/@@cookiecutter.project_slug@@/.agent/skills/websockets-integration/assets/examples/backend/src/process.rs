use backend::{get_sub, read_request, write_response, WebsocketConnections, load_aws_config};
...
struct AppState {
    ...
    queue_url: backend::shared::aws_config::SsmParameter,
}
...
async fn main() -> Result<(), Error> {
    ...
    let queue_url = backend::shared::aws_config::SsmParameter::new(&aws_config, "/@@ cookiecutter.project_slug @@/process-queue-url");
...
async fn function_handler(req: Request, state: AppState) -> Result<Response<Body>, Error> {
    ...
    state.sqs_client
        .send_message()
        .queue_url(state.queue_url.get().await?)
